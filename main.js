...
document.getElementById("description").textContent = ''
  ...
  const matches = Object.keys(bingoCalls).filter(number => number.startsWith(input));

  // Display suggestions
  matches.forEach(match => {
      const suggestionItem = document.createElement("div");
      suggestionItem.textContent = `${match} - ${bingoCalls[match].nickname}`;
      suggestionItem.onclick = () => selectSuggestion(match);
...
function selectSuggestion(number) {
  document.getElementById("nameInput").value = number;
  document.getElementById("suggestions").style.display = "none";

  showNickname();// Display the nickname immediately
}
...
function showNickname() {
  ...
  if (bingoCalls[number]) {
      nicknameElement.textContent = `Name: ${bingoCalls[number].nickname}`;
      descriptionElement.textContent = `Description: ${bingoCalls[number].description}`;
  } else {
      nicknameElement.textContent = "Unknown Number";
      descriptionElement.textContent = "Please enter a valid bingo number (1-75).";
  }
}
